...
#   NEW FUNCTION BASED ON NEW FORMAT
def read_ndc_file(input_file):
    """Read NDC file and detect if it has headers and record_id column."""
    ndc_data = []
    
    with open(input_file, 'r') as file:
        # Try to detect if file has tab-separated columns
        first_line = file.readline().strip()
        file.seek(0)  # Reset to beginning
        
        # Check if first line looks like a header (contains "NDC" and "record_id")
        has_header = '\t' in first_line and 'NDC' in first_line.upper()
        
        if '\t' in first_line:
            # File has tab-separated columns
            for line in file:
                if has_header and line == first_line:
                ...
                if len(parts) >= 2:
                    ...
                    if validate_ndc(ndc):
                        ndc_data.append({'ndc': ndc, 'record_id': record_id})
                elif len(parts) == 1:
                    ndc = parts[0].strip()
                    if validate_ndc(ndc):
                        ndc_data.append({'ndc': ndc, 'record_id': None})
        else:
            # File has only NDC codes (one per line)
            for line in file:
                ndc = line.strip()
                if validate_ndc(ndc):
                    ndc_data.append({'ndc': ndc, 'record_id': None})
...
def process_ndc_list(input_file, output_file, mapping_type, cache):
    ...
    # Read NDCs and record_ids from input file
    ndc_data = read_ndc_file(input_file)
    ...
    try:
        for idx, entry in enumerate(ndc_data, start=1):
            ...
            if rxcui:
                if mapping_type == 'atc4':
                    ...
                    for atc_class in atc_classes:
                        result = {'NDC': ndc, 'record_id': record_id, 'ATC4 Class': atc_class}
                    ...
                    if atc5_classes:
                        ...
                        for atc_class in atc5_classes:
                            result = {'NDC': ndc, 'record_id': record_id, 'ATC5 Class': atc_class}
                            results.append(result)
                    else:
                        result = {'NDC': ndc, 'record_id': record_id, 'ATC5 Class': 'No ATC Mapping Found'}
                        results.append(result)
                else:
                ...
                if mapping_type == 'atc5':
                    result = {'NDC': ndc, 'record_id': record_id, 'ATC5 Class': 'No RxCUI Found'}
                    results.append(result)
    ...
    # Write results to CSV - always include record_id column
    with open(output_file, 'w', newline='') as csvfile:
        if mapping_type == 'atc4':
            fieldnames = ['NDC', 'record_id', 'ATC4 Class']
        elif mapping_type == 'atc5':
            fieldnames = ['NDC', 'record_id', 'ATC5 Class']
        else:
            print(f"Unknown mapping type: {mapping_type}")
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument('input_file', type=str, help='Input file containing NDCs (one per line) or tab-separated NDC and record_id.')
    ...
    print("Will start querying the NDCs. If you need to interrupt, press ].")
